...
wstring win32_error::format_message(_In_ int code)
{
    ...
    if (auto ch = ::FormatMessageW(FORMAT_MESSAGE_FROM_SYSTEM | FORMAT_MESSAGE_IGNORE_INSERTS, NULL, code, 0, const_cast<LPWSTR>(err.c_str()), SIZE_T_TO_INT(err.size()), NULL))
    {
        err.resize(ch);
        return err;
    }
