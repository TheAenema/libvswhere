...
{
public:
    typedef std::function<std::unique_ptr<Formatter>(CommandArgs&, VirtualConsole&)> FormatterFactory;
    typedef std::map<std::wstring, std::tuple<UINT, FormatterFactory>, ci_less> FormatterMap;

    static std::unique_ptr<Formatter> Create(_In_ const std::wstring& type, _In_ CommandArgs& args, _In_ VirtualConsole& console);
...
protected:
    ...
    Formatter(_In_ CommandArgs& args, _In_ VirtualConsole& console);
    ...
    VirtualConsole& VirtualConsole() const noexcept
    {
        return m_console;
    }
