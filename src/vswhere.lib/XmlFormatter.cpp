...
void XmlFormatter::StartDocument()
{
    VirtualConsole().WriteLine(
        ...
        VirtualConsole().Color(ColorTag),
        VirtualConsole().Color(ColorName),
        VirtualConsole().Color(ColorValue),
...
void XmlFormatter::StartScope(_In_opt_ const wstring& name, _In_ std::wstring::const_pointer fallback)
{
    ...
    if (name.empty())
    {
        m_scopes.push(XmlScope(pParent, VirtualConsole(), m_padding, fallback));
    }
    else
    {
        m_scopes.push(XmlScope(pParent, VirtualConsole(), m_padding, name));
    }
