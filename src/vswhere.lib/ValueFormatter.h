...
{
public:
    static std::unique_ptr<Formatter> Create(_In_ CommandArgs& args, _In_ ::VirtualConsole& console)
    {
        return std::unique_ptr<ValueFormatter>(new ValueFormatter(args, console));
    }

    ValueFormatter(_In_ CommandArgs& args, _In_ ::VirtualConsole& console) :
