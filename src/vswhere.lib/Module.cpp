...
const wstring& Module::get_Path() noexcept
{
    if (m_hModule && m_path.empty())
    {
        ...
        ::GetModuleFileNameW(m_hModule, const_cast<LPWSTR>(m_path.data()), SIZE_T_TO_INT(m_path.size()));
    }
...
const wstring& Module::get_FileVersion() noexcept
{
    ...
    if (!::GetFileVersionInfoW(path.c_str(), 0, SIZE_T_TO_INT(buffer.size()), buffer.data()))
    {
        return m_fileVersion;
    }
